...
namespace PathFinderDOTS.Data {
    [BurstCompile]
    public struct PathGridData : IDisposable {
        ...
        public PathGridData(int width, int height, float cellSize, float3 origin, float flyCostMultiplier,
            Allocator allocator) {
            Width = width;
            Height = height;
            CellSize = cellSize;
            Origin = origin;
            FlyCostMultiplier = flyCostMultiplier;
